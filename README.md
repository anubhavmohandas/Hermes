...
| 3 (3C) | Cron (durable SQLite scheduler, `.tick.lock`, 3-min interrupt, Mnemos write-back), Delegation (≤3 children, forbidden-tool restriction), Fetcher (Tavily/Firecrawl, SAFE_MODE, SSRF-every-hop), Connect (native MCP client + capability negotiation + PKCE OAuth) | ✅ Built, unit-tested · Ollama/Tavily/MCP live paths proven on real infra 2026-07-05; 6 failure modes (unreachable daemon, invalid key, dangerous/malformed/wedged MCP server) tested and fail closed as of 2026-07-05, two CLI robustness bugs found and fixed same session (`logs/proof_gate4.md`, `logs/proof_failuremode.md`, both local) — real-daemon-kill, genuinely-revoked-key, and live Apollo tier-fallback behavior remain untested |
