...
2. Build a short task description string and run:
   `python3 brain.py check --task "<description>" --model "<model you're about to use>"`
   (Those are the only two flags `check` accepts. A `--via api|local` argument was documented here until 2026-08-05 and never existed — argparse rejected the whole call, so following this step literally failed the tier check on every request. It was a leftover from the local-Ollama tier, which no longer exists.)
...
| review pending self-improvement proposals | `~/.claude/hermes/curator/pending/*.json` — read them, `curator/approve.py approve\|reject <id>` on the user's explicit decision only | Active (v1) |
...
| database / persist structured data / Supabase | `integrations/db/store.py` (Postgres if `DATABASE_URL` set, else SQLite fallback) | Active on demand (Stage 5) — dormant until called |
| download / extract frames / transcribe a video | `integrations/media.py` (yt-dlp/ffmpeg/whisper per-tool availability check, honest "not installed" instead of a fake result) | Active on demand (Stage 5) — dormant until called |
...
| be lazy / minimal code / yagni / simplest solution / stop over-engineering | `meta/occam.py` + `hooks/occam_activate.sh` (SessionStart, full ladder) + `hooks/occam_mode_tracker.sh` (UserPromptSubmit, per-turn reminder + `/occam lite\|full\|ultra\|off`) + `hooks/occam_subagent.sh` (propagates to Task subagents, `OCCAM_SUBAGENT_MATCHER` scoping). Default: full, every session | Active |
| review this diff / audit this repo for over-engineering / what can I delete | `skills/occam-review` (diff) / `skills/occam-audit` (whole repo) — ranked delete-list, applies nothing | Active |
| what did occam defer / occam debt / list the shortcuts | `skills/occam-debt` — greps `occam:` comments into a ledger | Active |
...
Usage:
- **State paths below are runtime state, and runtime state lives under `~/.claude/hermes/`** (`$CLAUDE_CONFIG_DIR/hermes/` when set) — not beside the code. Every module resolves them through `meta/paths.py`; the repo-relative forms this section used to print (`mnemos/vault/…`, `curator/pending/…`, `mnemos/dream/…`) were the pre-migration locations and pointed at directories a plugin update deletes. Ask a module for its path (`python3 cron/scheduler.py status`, `python3 integrations/db/store.py status`) rather than assuming one.
- Write every meaningful exchange: `python3 mnemos/store.py write "<session_id>" "<role>" "<content>" [memory_type]` AND `python3 mnemos/hnsw_index.py ~/.claude/hermes/mnemos/vault/hnsw insert "<content>"` — both stores, so all 3 tiers can find it later. That index dir is the one `hybrid_search.py` reads (it derives it as `db_path.parent/hnsw`); writing anywhere else produces entries Tier C can never return. Each message carries one of the blueprint's 4 memory types (`user` / `feedback` / `project` / `reference`); omit the argument and `store.py` classifies deterministically (keyword rules, not LLM — same philosophy as brain.py's sensitivity check). Pass it explicitly when you know better than the heuristic.
...
- The MEMORY.md index (`~/.claude/hermes/mnemos/vault/MEMORY.md`) is capped at 200 lines / 25KB — `mnemos/memory_index.py` enforces this. If you're about to write more than that into the index, you're supposed to fail loudly (truncation warning), not silently drop entries. Move detail into topic files instead of growing the index.
...
**Curator** (mistake capture): `curator/consolidate.py` reads the raw append-only `logs/reflexion_seed.json` and produces deduplicated, recurrence-counted `curator/reflexion.json`. `curator/propose.py` turns any failure that's recurred (recurrence_count >= 2) into a proposal written to `~/.claude/hermes/curator/pending/<id>.json`. **Nothing auto-applies.** A proposal sits in `pending/` until the user explicitly runs `curator/approve.py approve <id>` or `reject <id>` — Apollo NEVER calls approve.py on its own initiative, regardless of how confident it is the proposal is correct.
...
- **HERMES never auto-applies updates.** Curator proposals land in `~/.claude/hermes/curator/pending/` for human review — this is a real, working mechanism now (§4b), not a stated intention. Never call `curator/approve.py` yourself. Never act on a self-generated change without the user approving it first.
